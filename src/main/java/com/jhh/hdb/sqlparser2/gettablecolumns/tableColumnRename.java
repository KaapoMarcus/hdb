package com.jhh.hdb.sqlparser2.gettablecolumns;
...
public class tableColumnRename {
	...
	private class metaDB implements IMetaDatabase {
		...
		public boolean checkColumn(String pServer, String pDatabase,
			...
			return metaInfo.containsKey(tablePrefix.trim().toLowerCase())
					&& metaInfo.get(tablePrefix.toLowerCase()).contains(
							pColumn.trim().toLowerCase());
		}
	}
	...
	private void modifyTableName(TCustomSqlStatement stmt) {
		...
		for (int k = 0; k < stmt.tables.size(); k++) {
			...
			for (int m = 0; m < table.getLinkedColumns().size(); m++) {
				TObjectName column = table.getLinkedColumns().getObjectName(m);
				isfound = false;
				if (column.getTableString() != null
						&& column.getTableString().equalsIgnoreCase(
								this.sourceTable)) {
					isfound = true;
					if (this.sourceSchema != null) { // check schema of this
														// table
						if (column.getSchemaString() != null) {
							isfound = this.sourceSchema.equalsIgnoreCase(column
									.getSchemaString());
						} else {
							isfound = false;
						}
				...
				if (isfound) {
					...
					if (this.targetSchema != null) {
						if (column.getSchemaString() != null) {
							...
							column.getTableToken().setString(
									this.targetSchema + "."
			...
			if (table.getTableName() != null
					...
					&& table.getTableName().getTableString()
							.equalsIgnoreCase(this.sourceTable)) {
				isfound = true;
				if (this.sourceSchema != null) { // check schema of this table
					if (table.getTableName().getSchemaString() != null) {
						isfound = this.sourceSchema.equalsIgnoreCase(table
								.getTableName().getSchemaString());
					} else {
						isfound = false;
					}
				}
				...
				if (table.getTableName() != null) {
					...
					if (this.targetSchema != null) {
						if (table.getPrefixSchema() != null) {
							table.getTableName().getSchemaToken()
									.setString(this.targetSchema);
						} else {
							st.setString(this.targetSchema + "." + st.astext);
						}
	...
	private void modifyColumnName(TCustomSqlStatement stmt) {
		...
		for (int k = 0; k < stmt.tables.size(); k++) {
			...
			isThisTable = table.getTableName().getTableString()
					.equalsIgnoreCase(this.sourceTable);
			if (!isThisTable) {
				continue;
			}
			...
			if (this.sourceSchema != null && table.getTableName() != null) {
				if (table.getTableName().getSchemaString() != null) {
					isThisTable = table.getTableName().getSchemaString()
			...
			for (int m = 0; m < table.getLinkedColumns().size(); m++) {
				...
				if (column.getColumnToken() != null) {
					if (column.getColumnNameOnly().equalsIgnoreCase(
							this.sourceColumn)) {
	...
	public static void main(String[] args) {
		...
		metaInfo.put("dbo.tb_Seasons".toLowerCase(),
				Arrays.asList(new String[] { "MinimalRentalID".toLowerCase() }));
		tableColumnRename ro = new tableColumnRename(EDbVendor.dbvmssql, text,
				metaInfo);
